...
use std::{collections::HashSet, path::Prefix};
...
use crate::{expiration::Expiration, state::OFFERS, token::{self, Extension, Trait}};
...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> StdResult<Response> {
    ...
    let response = match msg {
        ...
        // **** Hackathon
        ExecuteMsg::CreateCreature { name, secret_name } => {
            create_creature(
                deps,
                &env,
                &info.sender,
        ...
        ExecuteMsg::OfferRemixing { my_token_id, other_token_id } => {
            offer_remix(
                deps,
                &env,
                &info.sender,
        ...
        ExecuteMsg::CompleteRemixing { my_token_id, other_token_id } => {
            complete_remix(
                deps,
                &env,
                &info.sender,
...
fn create_trait(trait_type: &str, value: &str) -> Trait {
    Trait {
        trait_type: Some(trait_type.to_string()),
...
pub fn create_creature(
    ...
    name: String,
    secret_name: String,
    // other params (update msg as well...)
) -> StdResult<Response> {
    check_status(config.status, priority)?;
    let sender_raw = deps.api.addr_canonicalize(sender.as_str())?;

    // random number generator
    // use this to create random selections of attributes
    let mut prng = ContractPrng::from_env(env);
    let _a_u64_number: u64 = prng.next_u64();

    // Does creator select traits or are they all randomly selected?
    // Maybe genome randomly generated and traits selected from that?

    // Creature traits - we need to add a randomisation to this though!
    // public
    let ear = match prng.next_u64() % 100 {
        0..=19 => 1,
        20..=29 => 2,
        30..=59 => 3,
        60..=64 => 4,
    ...
    let face = match prng.next_u64() % 100 {
        0..=19 => 1,
        20..=29 => 2,
        30..=59 => 3,
        60..=64 => 4,
    ...
    let body = match prng.next_u64() % 100 {
        0..=19 => 1,
        20..=29 => 2,
        30..=59 => 3,
        60..=64 => 4,
    ...
    let tail = match prng.next_u64() % 100 {
        0..=19 => 1,
        20..=29 => 2,
        30..=59 => 3,
        60..=64 => 4,
    ...
    let colour = match prng.next_u64() % 100 {
        0..=19 => "orange",
        20..=29 => "black_and_white",
        30..=59 => "grey",
        60..=64 => "brown",
        65..=69 => "pink",
        70..=99 => "purple",
    ...
    let coat = match prng.next_u64() % 100 {
        0..=19 => "solid",
        20..=29 => "blotchy",
        30..=59 => "tabby",
        60..=99 => "rainbow",
    ...
    // private 
    let temperament = match prng.next_u64() % 100 {
        0..=19 => "shy",
        20..=29 => "feisty",
        30..=59 => "calm",
        60..=69 => "aggressive",
    ...
    let public_metadata = Metadata {
        ...
        extension: Some(Extension {
            ...
            image_data: None,
            external_url: None,
            description: Some("put a public description here".to_string()),
            name: Some(name),
            attributes: Some(vec![
                create_trait("ears", &ear.to_string()),
                create_trait("face", &face.to_string()),
                create_trait("body", &body.to_string()),
                create_trait("tail", &tail.to_string()),
                create_trait("colour", colour),
                create_trait("coat", coat),
            ]),
    ...
    let private_metadata = Metadata {
        ...
        extension: Some(Extension {
            ...
            image_data: None,
            external_url: None,
            description: Some("put a private description here".to_string()),
            name: Some(secret_name),
            attributes: Some(vec![
                create_trait("temperament", temperament),
            ]),
            background_color: None,
            animation_url: None,
            youtube_url: None,
    ...
    Ok(Response::new()
        ...
        .set_data(to_binary(&ExecuteAnswer::CreateCreature {
            token_id: minted_str,
        })?))
}
...
pub fn offer_remix(
    ...
    let custom_err = format!("Not authorized to remix token {}", my_token_id);
    ...
    let (token, _idx) = get_token(deps.storage, my_token_id.as_str(), opt_err)?;
    if sender_raw != token.owner {
        return Err(StdError::generic_err(custom_err));
    }

    // maybe check that the owner of the other_token_id is not also the sender?

    // need to check if already offered (store this in storage)
    // probably only want to allow one offer at a time for each token
    
    // store offer on the chain if not already offered

    let offer_store = OFFERS.add_suffix(other_token_id.as_bytes());
    match offer_store.may_load(deps.storage) {
        Ok(a) => match a {
            Some(mut val) => {
                val.push(my_token_id);
                offer_store.save(deps.storage, &val)?;
            },
            None => {
                offer_store.save(deps.storage, &vec![my_token_id])?;
            }
        },
        Err(_) => {
            return Err(StdError::generic_err("Unable to load offer store"));
        }
    }

    Ok(Response::new()
        .set_data(to_binary(&ExecuteAnswer::OfferRemixing { status: Success })?))
}
...
pub fn query_offers(
    ...
    let offer_store = OFFERS.add_suffix(token_id.as_bytes());
   ...
   to_binary(&QueryAnswer::Offer { my_token: token_id, other_token: tokens })
}

fn remix_traits(t1: &Trait, t2: &Trait, env: &Env) -> Trait {
    let mut prng = ContractPrng::from_env(env);

    if t1.trait_type.as_ref().unwrap().contains("colour") {
        let colour = match prng.next_u64() % 100 {
            0..=39 => t1.value.clone(),
            40..=79 => t2.value.clone(),
            80..=99 => match prng.next_u64() % 100 {
                0..=19 => "orange",
                20..=29 => "black_and_white",
                30..=59 => "grey",
                60..=64 => "brown",
        ...
        return create_trait("colour", &colour)
    }

    if t1.trait_type.as_ref().unwrap().contains("coat") {
        let t1_rainbow = t1.value.clone().contains("rainbow");
        let t2_rainbow = t2.value.clone().contains("rainbow");

        let mut value = match prng.next_u64() % 100 {
            0..=39 => t1.value.clone(),
            40..=79 => t2.value.clone(),
            90..=99 => match prng.next_u64() % 100 {
                0..=35 => "solid",
                36..=60 => "blotchy",
                61..=98 => "tabby",
                99 => "rainbow",
        ...
        if t1_rainbow && t2_rainbow {
            value = match prng.next_u64() % 100 {
                0..=28 => "solid",
                29..=56 => "blotchy",
                57..=83 => "tabby",
                84..=99 => "rainbow",
        ...
        } else if t1_rainbow || t2_rainbow {
            value = match prng.next_u64() % 100 {
                0..=79 => if t1_rainbow { t2.value.clone() } else { t1.value.clone() }
                80..=94 => match prng.next_u64() % 100 {
                    0..=35 => "solid",
                    36..=60 => "blotchy",
                    61..=98 => "tabby",
                    99 => "rainbow",
                ...
                95..=99 => "rainbow".to_string(),
                _ => unreachable!(),
            }
        }

        return create_trait(&t1.trait_type.clone().unwrap(), &value);
    }

    let t1_val: i32 = t1.value.parse().unwrap();
    let t2_val: i32 = t2.value.parse().unwrap();

    let dominant = if t1_val > t2_val { t1_val } else { t2_val };
    let recessive = if dominant == t1_val { t2_val } else { t1_val };

    let value = match prng.next_u64() % 100 {
        0..=74 => dominant,
        75..=98 => recessive,
        99 => match prng.next_u64() % 100 {
                0..=15 => 1,
                16..=31 => 2,
                32..=47 => 3,
                48..=64 => 4,
    ...
    create_trait(&t1.trait_type.clone().unwrap(), &value.to_string())
}

pub fn complete_remix(
    deps: DepsMut,
    env: &Env,
    sender: &Addr,
    config: &mut Config,
    ...
    let (token, idx) = get_token(deps.storage, my_token_id.as_str(), opt_err)?;
     if sender_raw != token.owner {
        return Err(StdError::generic_err(custom_err));
    }

    let offer_store = OFFERS.add_suffix(my_token_id.as_bytes());
    let offered_tokens = offer_store.may_load(deps.storage)?;
    if let Some(mut offered_tokens) = offered_tokens {
        match offered_tokens.binary_search(&other_token_id) {
            Ok(idx) => {
                offered_tokens.remove(idx);
                offer_store.save(deps.storage, &offered_tokens)?;
            },
            Err(_) => {
                return Err(StdError::generic_err("Not Offered"));
            }
        }
    } else {
        return  Err(StdError::generic_err("not offered"));
    ...
    // check if other_token_id has offered for my_token_id
    let (_token2, idx2) = get_token(deps.storage, other_token_id.as_str(), opt_err)?;
    
    let pub_meta_store_t1 = PrefixedStorage::new(deps.storage, PREFIX_PUB_META);
    let pub_meta_t1 = may_load(&pub_meta_store_t1, &idx.to_le_bytes())?.unwrap_or(Metadata {
        token_uri: None,
        extension: None,
    });

    let pub_meta_store_t2 = PrefixedStorage::new(deps.storage, PREFIX_PUB_META);
    let pub_meta_t2 = may_load(&pub_meta_store_t2, &idx2.to_le_bytes())?.unwrap_or(Metadata {
        token_uri: None,
        extension: None,
    });

    let traits_t1 = pub_meta_t1.extension.unwrap().attributes.unwrap();
    let traits_t2 = pub_meta_t2.extension.unwrap().attributes.unwrap();

    let traits: Vec<Trait> = traits_t1.iter().zip(traits_t2.iter()).map(|(t1, t2)| remix_traits(t1, t2, env)).collect();
    ...
    let private_metadata = Metadata {
        ...
        extension: Some(Extension {
            ...
            attributes: Some(vec![
                Trait {
                    trait_type: Some("temperament".to_string()),
                    value: "sad".to_string(),  // shy, fiesty, etc.
                    display_type: None,
                    max_value: None,
                },
                // maybe genome goes here?
                // ...
            ]),
            background_color: None,
            animation_url: None,
...
pub fn mint(
    ...
    _private_metadata: Option<Metadata>,
    _serial_number: Option<SerialNumber>,
    _royalty_info: Option<RoyaltyInfo>,
    _transferable: Option<bool>,
    _memo: Option<String>,
) -> StdResult<Response> {
    // **** Hackathon
